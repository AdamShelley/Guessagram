...
export default function Stats({
  ...
  const { daysPlayed, totalWords, totalScore, totalGuessAttempts } = statisticsLocalStorage;
    ...
    >
      <div className="text-white absolute bg-slate-800 border border-slate-700 w-11/12  top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-3/4 p-4 rounded-sm flex flex-col gap-3 lg:w-5/12">
        <FontAwesomeIcon
          ...
          icon={faXmark}
        />
        <h2 className="px-8 text-left text-xl text-white">Statistics</h2>

        <ul className="text-left text-sm md:text-lg p-8">
          <li className="mt-4 text-md">Days Played: <span className="font-bold text-white text-xl ml-5">{daysPlayed}</span></li>
          <li className="mt-4 text-md">Total words guessed: <span className="font-bold text-white text-xl ml-5">{totalWords}</span></li>
          <li className="mt-4 text-md">Total correct words: <span className="font-bold text-white text-xl ml-5">{0}/{totalGuessAttempts}</span></li>
          <li className="mt-4 text-md">Total Score: <span className="font-bold text-white text-xl ml-5">{totalScore}</span></li>
        </ul>
      </div>
    </div>
  );
